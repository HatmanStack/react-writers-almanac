...
- **Biggest operational risk:** The `search-autocomplete` Lambda uses module-level mutable state (`authorSlugsCache`, `cacheTimestamp`) which is non-deterministic across Lambda invocations and can serve stale data indefinitely when instances are reused.
...
3. **[Architectural Debt]** `frontend/src/hooks/usePoemData.ts:62-181` and `frontend/src/hooks/queries/usePoemQuery.ts`
...
4. **[Structural Debt]** `backend/lambdas/get-author/index.js:47-55`, `backend/lambdas/get-authors-by-letter/index.js:29-39`, `backend/lambdas/search-autocomplete/index.js:39-47`
...
6. **[Operational Debt]** `frontend/src/hooks/usePoemData.ts:138-152`
...
10. **[Code Hygiene Debt]** `frontend/src/utils/debug.ts:1-149`
...
13. **[Operational Debt]** `frontend/src/api/client.ts:32`
...
15. **[Architectural Debt]** `frontend/src/components/Search.tsx:167` and `frontend/src/components/Search.tsx:253`
...
17. **[Code Hygiene Debt]** `frontend/src/hooks/usePoemData.ts:83-91`
...
22. **[Code Hygiene Debt]** Zero test coverage for backend Lambda functions
...
### QUICK WINS
1. `frontend/src/components/Search.tsx:167,253` — Replace hardcoded `maxDate`/`minDate` with imports from `DATE_BOUNDARIES` in `utils/dateMapping.ts` (estimated effort: < 15 minutes)
2. `frontend/src/components/Poem.tsx:46-47` — Change `==` and `!=` to `===` and `!==` (estimated effort: < 5 minutes)
3. `frontend/src/App.tsx:392` — Remove the empty `<div className="FormattingContainer" />` (estimated effort: < 5 minutes)
...
**Dead code scan:**
- `frontend/src/utils/debug.ts` — Exported functions not imported in any production source file
- `frontend/src/hooks/queries/usePoemQuery.ts` — Hook exists but is not used in any component
